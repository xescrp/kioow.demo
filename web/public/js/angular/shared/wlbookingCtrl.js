...
///
            ...
            $scope.selectedpaymentoption = 'tpv-100';
            $scope.paysplited = function () { return $scope.selectedpaymentoption == 'tpv-split'; }
...
///
			...
			//	$scope.booking.roomDistribution = bookings_service.addRoom($scope.booking.roomDistribution);
            ...
            $scope.addRoom = function () {
                $scope.rooms = bookings_service.addRoom($scope.rooms);
                $scope.loadBooking();
                document.dispatchEvent(new CustomEvent("sticky:recalc"));
            }
...
///
            ...
            $scope.urltpv = '/booking/tpvframe/' + $scope.idbooking;
            //DO THE BOOKING...
            $scope.confirmBtn = function (callback) {
                if (customerDataIsValid() && $scope._checkBooking()) {
                    ...
                    rqCB.on(rqCB.oncompleteeventkey, function (rsp) {
                        if (rsp != null) {
                            ...
                            tools_service.showPreloader($scope, 'hide');
                            console.log(rsp);
                            $scope.idbooking = rsp.idBooking;
                            $scope.urltpv = '/booking/tpvframe?idbooking=' + $scope.idbooking;
                            //actualizo el id de reserva
                            //$scope.newbooking = rsp;
                            //$scope.roomcodecount = $scope.roomcount();
                            //$scope.paxescount();
                    ...
                    rqCB.on(rqCB.onerroreventkey, function (err) {
                        tools_service.showPreloader($scope, 'hide');
                        toaster.pop('error', "Error de conexión",
                            "No se ha podido guardar tu reserva. Por favor intenta más tarde.", 10000);
                        //$scope.errorServerSave = true;
                        $scope.readyToPay = false;
                        $log.error(err);
                        callback != null ? callback() : null;
...
///
            ...
            $scope.paxescount = function () {
                ...
                $scope.roompaxescount.total = $scope.roompaxescount.triple +
                    $scope.roompaxescount.double + $scope.roompaxescount.single;
            }
			...
			$scope.findCountryByCode = function(countryCode){
				return bookings_service.findcounty(countryCode).name_es;
			};
...
///
			...
			$scope.feedbackError = function () {
				...
				if(!StringUtils.emailIsValid($scope.booking.signin.email))
					errors.push('El email introducido no es válido.');

				if(!StringUtils.emailsAreEqual($scope.booking.signin.email, $scope.booking.signin.reemail))
			...
			var _getProduct = function(callback){
				...
				var rqprod = {
					...
					request: {
                        ...
                        populate: [{ path: 'dmc', select: 'code name images company.name additionalinfo.description additionalinfo.description_es membership.b2bchanel membership.commission membership.b2bcommission membership.cancelpolicy._es membership.paymentoption membership.pvp currency'}]
					}
				};
