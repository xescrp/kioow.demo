app.controller("resultsCtrl",
    ...
    function(
        ...
        $scope.viewblocks = tools_service.browserMobile();
    ...
    function weekdayhash(operationdays) {
        var hashed = null;
        if (operationdays != null && operationdays.length > 0) {
            hashed = _.map(operationdays, function (weekday) {
                return weekhash[weekday];
            });
        }
        return hashed;
    }
    ...
    $scope.getquery = function () {
        ...
        $scope.filtersProduct.operationdays = querystring.operationdays != null && querystring.operationdays != '' ? querystring.operationdays.split(',') : null;
    ...
    $scope.setquery = function () {
        ...
        ($scope.filtersProduct.operationdays != null && $scope.filtersProduct.operationdays.length > 0) ?
            $location.search('operationdays', $scope.filtersProduct.operationdays.join(',')) : $location.search('operationdays', null);
    ...
    $scope.buildquery = function (filter) { 
        ...
        if (filter.operationdays != null && filter.operationdays.length > 0) {
            $scope.filtersearch.operationdays = weekdayhash(filter.operationdays);
        }
    ...
    $scope.resetfilters = function () { 
        ...
        $location.search('operationdays', null);
    }
    ...
    $scope.hascategories = function (product) {
        ...
        return (product.categoryname != null && product.categoryname.related != null && product.categoryname.related.length > 1);
    ...
    $scope.likesearchinput = '';

    $scope.likesearch = function (callback, errorcallback) {
        var rq = {
            command: 'search2',
            service: 'api',
            request: { $like: $scope.likesearchinput }
        };
        tools_service.showPreloader($scope, 'show');
        var rqCB = yto_api.send(rq);
        console.log(rqCB);
...
app.directive("resultitem",['tools_service', '$uibModal', 'yto_api', '$cookieStore', 'Lightbox', function (tools_service, $uibModal, yto_api, $cookieStore, Lightbox) {
    return {
        templateUrl: '/partials/search/resultItem.html',
        scope: {
            result: '=resultdata',
            //bindAttr: '='
        },
            ...
            $scope.showGallery = function(code) {
                ...
                var rqprodimg = {
                    ...
                    request: {
                        ...
                        populate : [{path: 'dmc', select : 'code name images company.name membership.b2bcommission currency'}]
                    }
                };
