...
function location(list, path, options) {
    ...
    this.enableMapsAPI = omtcore.get('google api key') ? true : false;
    if (!options.defaults) {
        options.defaults = {};
    }
    if (options.required) {
...
function doGoogleGeocodeRequest(address, region, callback) {
    ...
    gmaps.GetGeoCode(address, function(rsmatrix){
        var result = rsmatrix;
     ...
     * Please make sure your omtcore app complies with the Google Maps API License.
     * 
...
location.prototype.googleLookup = function (item, region, update, callback) {
    ...
    doGoogleGeocodeRequest(address, region || omtcore.get('default region'), function (err, geocode) {
