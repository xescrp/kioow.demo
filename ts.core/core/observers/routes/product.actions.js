module.exports = function (subscriberworker, socket) {
    socket.on('new', function (data) {
        ...
        var eventkey = 'product.new.done';
        var erroreventkey = 'product.new.error';
        ...
        var rt = subscriber.processrequest(rq);
        
        rt.on(eventkey, function (rs) { console.log(rs); subscriber.setfinished(rs); });
        rt.on(erroreventkey, function (err) { console.log(err); subscriber.seterror(err); });

    });

    socket.on('update', function (data) {
        
        var eventkey = 'product.update.done';
        var erroreventkey = 'product.update.error';
        ...
        data.oncompleteeventkey = eventkey;
        data.onerroreventkey = erroreventkey;
        data.core = subscriberworker.core;
        data.code = data;
        ...
        rt.on(eventkey, function (rs) { console.log(rs); subscriberworker.setfinished(rs); });
        rt.on(erroreventkey, function (err) { console.log(err); subscriberworker.seterror(err); });
    });
    ...
    socket.on('cityupdate', function (data) {
        ...
        var core = subscriberworker.core;
        var started = new Date();
        console.log('Subscriber task started at ' + started);
        ...
        cev.on('product.ready', function (result) {
            if (result != null && result.product != null) {
                ...
                result.product.save(function (err, doc) {
                    err != null ? console.log(err) : console.log('Migration Product ' + doc.code + ' processed and saved');
                    cev.emit('product.next');
                    icts.migration++;
                });
            }
                ...
                dmcproduct.getItineraryCollections(product.code, function (itinerarycol) {
                    ...
                    process.nextTick(function () {
                        _.each(itinerarycol.sleepcities, function (sleep) {
                            var sleepcity = hashcities[sleep.slug] || hashcitiesids[sleep.cityid];
                            sleepcity != null ? counter.sleeps[sleep.slug] = sleepcity : null;
                            counter.sleep--;
                        ...
                        _.each(itinerarycol.departurecities, function (departure) {
                            var departurecity = hashcities[departure.slug] || hashcitiesids[departure.cityid];
                            departurecity != null ? counter.departures[departure.slug] = departurecity : null;
                            counter.departure--;
                            //counter.done() ? cev.emit('product.ready', { product: product, results: counter, original: dmcproduct }): null;
                            counter.done() ? cev.emit('product.processoriginal', { product: product, results: counter, original: dmcproduct }): null;
                        });
                        _.each(itinerarycol.stopcities, function (stop) {
                            var stopcity = hashcities[stop.slug] || hashcitiesids[stop.cityid];
                            stopcity != null ? counter.stops[stop.slug] = stopcity : null;
                            counter.stop--;
                            //counter.done() ? cev.emit('product.ready', { product: product, results: counter, original: dmcproduct }): null;
                            counter.done() ? cev.emit('product.processoriginal', { product: product, results: counter, original: dmcproduct }): null;
                        });
        ...
        cev.on('product.start', function () {
            core.list('DMCProducts').model.find({ code: productcode }).select('code').exec(function (err, productcodes) {
        ...
        cev.on('city.get', function (cities) {
            ...
            _.each(cities, function (city) {
                ...
                hashcitiesids[city._id.toString()] = city;
                ct--;
        ...
        cev.on('finish.done', function (results) {
            ...
            subscriberworker.setfinished({
                ResultOK: true,
                Message: 'Product cities updated',
                Results: results
            });
        });
        //find all countries
