module.exports = function (raditzseed, socket) {
    ...
    socket.on('pay', function (data) {
        ...
        var eventkey = 'pay.done';
        var erroreventkey = 'pay.error';
        var rq = {
            request: conf,
            method: 'pay'
        };
        var rt = raditzseed.processrequest(rq);
        rt.on(eventkey, function (rs) { raditzseed.trigger.emit('dispatch.done', { action: 'pay', data: data, result: rs }); });
        rt.on(erroreventkey, function (rs) { raditzseed.trigger.emit('dispatch.error', { action: 'pay', data: data, result: rs }); });
    });

    socket.on('charge', function (data) {
        var conf = { core: raditzseed.core, data: data, results: [] };

        var eventkey = 'charge.done';
        var erroreventkey = 'charge.error';
        var rq = {
            request: conf,
            method: 'charge'
        };
        var rt = raditzseed.processrequest(rq);
        rt.on(eventkey, function (rs) { raditzseed.trigger.emit('dispatch.done', { action: 'charge', data: data, result: rs }); });
        rt.on(erroreventkey, function (rs) { raditzseed.trigger.emit('dispatch.error', { action: 'charge', data: data, result: rs }); });
    });
