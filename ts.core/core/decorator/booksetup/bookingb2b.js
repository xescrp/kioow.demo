module.exports = function (conf, callback, errorcallback) {
    ...
    var affiliate = conf.affiliate || { fees: { pvpkeep: true }, membership: { omtmargin: 3 }  };
    ...
    //check a fee change...
    (conf.fee != null && conf.fee > 0) ? booking.pricing.fee = conf.fee : null;
    ...
    if (proceed) { //find the month and day for avail
        ...
        if (roomprices != null) {
            ...
            booking.pricing.rooms.double = Math.round(pricingrooms.double / (1 - (booking.pricing.fee / 100))); //: booking.pricing.roomssnapshot.double;
            booking.pricing.rooms.single = Math.round(pricingrooms.single / (1 - (booking.pricing.fee / 100))); //: booking.pricing.roomssnapshot.single;
            booking.pricing.rooms.triple = Math.round(pricingrooms.triple / (1 - (booking.pricing.fee / 100))); //: booking.pricing.roomssnapshot.triple;
            booking.pricing.rooms.quad = Math.round(pricingrooms.quad / (1 - (booking.pricing.fee / 100))); //: booking.pricing.roomssnapshot.quad;
