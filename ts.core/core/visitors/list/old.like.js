...
module.exports = function (options, callback, errorcallback) { 
    ...
    var totalitems = 0;
    var finded = 0;
    var _query = options.query || { $or : [{ _id : { $ne: null } }] };
    for (var i = 0, len = _query.$or.length; i < len; i++) {
        for (var prop in _query.$or[i]) {
            var text = _query.$or[i][prop];
            _query.$or[i][prop] = new RegExp(text, "i");
        }
    }
